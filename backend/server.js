...
// express-mongo-sanitize removed due to compatibility with Express v5
...
app.get('/api/items', async (req, res) => {
  try {
    ...
    if (!items.length) {
      await Item.insertMany([...demoMusicalEvents, ...demoOtherEvents]);
      items = await Item.find().sort({ date: 1 });
    } else {
      const existingMusicalSubCategories = new Set(
        items
          .filter(event => event.category?.toLowerCase() === 'musical')
          .map(event => (event.subCategory || 'Autre').toLowerCase())
      );

      const missingMusicalEvents = demoMusicalEvents.filter(
        event => !existingMusicalSubCategories.has(event.subCategory.toLowerCase())
      );

      if (missingMusicalEvents.length) {
        await Item.insertMany(missingMusicalEvents);
        items = await Item.find().sort({ date: 1 });
      }
    }
...
app.post('/api/items', authenticateToken, async (req, res) => {
  try {
    ...
    await newItem.save();
    res.status(201).json({ success: true, message: 'Événement créé avec succès.', data: newItem });
  } catch (error) {
...
app.get('/api/organizer-dashboard', authenticateToken, async (req, res) => {
  try {
    // Determine organizer from token if available, otherwise fall back to query
