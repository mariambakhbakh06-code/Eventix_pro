...
function generatePayPalCheckoutUrl(ticketCode, totalPrice) {
  // Replace with your business or sandbox email if needed
  const businessEmail = encodeURIComponent("merchant@eventix.com"); 
  const itemName = encodeURIComponent(`Eventix Ticket - ${ticketCode}`);
  
  // Standard PayPal URL format for a simple "Buy Now" checkout button link
  return `https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=${businessEmail}&item_name=${itemName}&amount=${totalPrice}&currency_code=MAD&custom=${ticketCode}`;
}
...
const itemSchema = new mongoose.Schema({
  ...
  maxTicketsPerPurchase: { type: Number, default: 4 },
  status: {
    type: String,
  ...
  organizerPaypalEmail: { type: String }, // ✅ ADDED - Organizer's PayPal to receive payouts
...
const bookingSchema = new mongoose.Schema({
  ...
  paymentStatus: {
    type: String,
    enum: ['unpaid', 'completed', 'refunded'],
    default: 'unpaid'
  },
...
// Customer Schema - FIXED: passwordHash is now optional
...
// Organizer Schema - UPDATED with paypalEmail
const organizerSchema = new mongoose.Schema({
  ...
  paypalEmail: { type: String, required: true }, // ✅ ADDED - Where to send payouts
  status: {
    type: String,
...
app.post("/api/organizers/signin", async (req, res) => {
  try {
    ...
    if (!paypalEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(paypalEmail)) {
      return res.status(400).json({
        success: false,
        message: "Valid PayPal email is required to receive payments"
      });
    }
    ...
    const organizer = new Organizer({
      ...
      paypalEmail: paypalEmail.trim(), // ✅ ADDED
      status: "pending"
    });
      ...
      if (parsed.isYes) {
        ...
        try {
          ...
          if (bookingResult.success) {
            ...
            return {
              reply: `✅ **Almost done!** Your booking is reserved.\n\nTicket Code: ${bookingResult.data.ticketCode}\nEvent: ${bookingResult.data.eventName}\nTotal: ${bookingResult.data.totalPrice} MAD\n\n💳 **Complete your payment via PayPal to confirm your ticket:**\n${bookingResult.data.paypalUrl}\n\n⚠️ Your seats will be confirmed only after payment is received. You'll get a QR code ticket by email once payment is complete.\n\nType 'find events' to search again.`,
              shouldSearch: false
            };
          } else {
            return {
...
app.post('/api/paypal-webhook', async (req, res) => {
  try {
    ...
    console.log("📨 Webhook received from Zoho Flow:", webhookData);
    ...
    const { 
      ...
      payer_email, 
      payment_status,
      custom_id  // this is the ticketCode set in generatePayPalCheckoutUrl
    } = webhookData;
    
    if (payment_status === 'COMPLETED') {
      console.log(`✅ Payment completed for transaction: ${transaction_id}`);

      // ✅ Find the pending booking by ticketCode
      const booking = await Booking.findOne({ ticketCode: custom_id, paymentStatus: 'unpaid' })
        .populate('eventId');

      if (!booking) {
        console.warn(`⚠️ No unpaid booking found for ticketCode: ${custom_id}`);
        return res.status(200).json({ success: true, message: "No matching booking" });
      }

      // ✅ Verify the paid amount matches expected total (prevent underpayment)
      if (parseFloat(amount) < booking.totalPrice) {
        console.error(`❌ Payment amount mismatch: expected ${booking.totalPrice}, got ${amount}`);
      ...
      booking.paypalTransactionId = transaction_id;
      await booking.save();

      // ✅ Now decrement available seats
      const event = booking.eventId;
      if (event) {
        if (event.availableSeats < booking.totalQuantity) {
          console.error(`❌ Not enough seats for booking ${custom_id} after payment`);
          // Edge case: seats ran out between reservation and payment.
          // Refund logic should be triggered here in production.
        } else {
          event.availableSeats -= booking.totalQuantity;
          await event.save();
        }
      }

      // ✅ Update customer booking count
      const customer = await Customer.findOne({ email: booking.contactPerson.email });
      if (customer) {
        customer.totalBookings += 1;
        await customer.save();
      }

      // ✅ Now send the confirmation ticket email with QR code
      const eventDate = new Date(event.date).toLocaleDateString('en-US', {
      ...
      const bookingDetails = {
        ...
        customerName: `${booking.contactPerson.firstName} ${booking.contactPerson.lastName}`,
        attendees: booking.attendees
      };
      const { qrCodeBuffer, ticketUrl } = await generateTicketQRCode(booking.ticketCode, bookingDetails);
      const emailResult = await sendTicketEmail(booking.contactPerson.email, bookingDetails, qrCodeBuffer, ticketUrl);
      if (!emailResult.success) {
        console.warn('⚠️ Ticket email failed after confirmed payment:', emailResult.error);
      }

      console.log(`🎉 Booking ${custom_id} fully confirmed after PayPal payment.`);
    }
...
async function processBooking(bookingData) {
  try {
    ...
    const booking = new Booking({
      ...
      ticketCode,
      attendees,
      bookingStatus: 'pending',   // ✅ stays pending until PayPal payment confirmed
      paymentStatus: 'unpaid'     // ✅ must be paid before confirmed
    });

    await booking.save();

    // ✅ Seats are NOT decremented yet — they are held only after payment succeeds
    // (decremented in the PayPal webhook once payment is COMPLETED)

    const paypalUrl = generatePayPalCheckoutUrl(ticketCode, totalPrice);
    ...
    return {
      ...
      data: {
        ...
        eventName: event.name,
        totalPrice,
        paypalUrl   // ✅ returned to chatbot so user is directed to pay
      }
    };
...
app.post('/api/customers/signin', async (req, res) => {
  try {
    let { firstName, lastName, email, phone, acceptTerms, password, isOrganizer } = req.body;
    ...
    const token = jwt.sign(
      { id: customer._id, email: customer.email, role: customer.isOrganizer ? 'organizer' : 'customer' },
    ...
    res.json({
      ...
      data: {
        ...
        email: customer.email,
        phone: customer.phone,
        role: customer.isOrganizer ? 'organizer' : 'customer'
      },
      token
    });
...
app.post('/api/booking', async (req, res) => {
  try {
    const { customerEmail, eventId, attendees, paymentId, paymentStatus } = req.body;
    ...
    if (!customerEmail || !emailRegex.test(customerEmail)) {
      return res.status(400).json({ success: false, message: 'Customer email required and must be valid' });
    }

    if (!eventId || !mongoose.Types.ObjectId.isValid(eventId)) {
      return res.status(400).json({ success: false, message: 'Valid eventId required' });
    }

    if (!Array.isArray(attendees) || attendees.length === 0) {
      return res.status(400).json({ success: false, message: 'At least one attendee required' });
    }
    ...
    if (!customer) {
      ...
      await customer.save();
      console.log("✅ Auto-created guest customer:", customer.email);
    }
    ...
    const limit = event.maxTicketsPerPurchase || 4;
    if (attendees.length > limit) {
      return res.status(400).json({ success: false, message: `Vous ne pouvez pas réserver plus de ${limit} tickets par transaction pour cet événement.` });
    }
...
// Demo events data
const demoMusicalEvents = [
  {
    name: 'Jazz Night Casablanca',
    description: 'Une soirée élégante dédiée au jazz moderne et aux standards classiques.',
    date: '2026-07-05T20:00:00Z',
    time: '20:00',
    venue: 'Jazz Club Casablanca',
    city: 'Casablanca',
    price: 180,
    totalSeats: 120,
    availableSeats: 120,
    ...
    category: 'musical',
    subCategory: 'Jazz',
    organizerName: 'EVENTIX Music',
    organizerEmail: 'music@eventix.com',
    organizerPaypalEmail: 'music@eventix.com',
    maxTicketsPerPurchase: 4,
    status: 'approved'
  },
  {
    name: 'Rock Live Festival',
    description: 'Groupe live, guitares brûlantes et ambiance rock jusqu’au bout de la nuit.',
    ...
    totalSeats: 500,
    availableSeats: 500,
    imageUrl: 'https://images.unsplash.com/photo-1518977956816-2d21aeb66b79?auto=format&fit=crop&w=800&q=80',
    category: 'musical',
    subCategory: 'Rock',
    organizerName: 'EVENTIX Music',
    organizerEmail: 'music@eventix.com',
  ...
  {
    name: 'Rap Battle Night',
    description: 'Les meilleurs rappeurs se rencontrent pour une soirée de battles et freestyle.',
  ...
  {
    name: 'Classique à l’Opéra',
    description: 'Un concert de musique classique avec orchestre symphonique et solistes invités.',
  ...
  },
  {
    ...
    description: 'Atelier pratique pour apprendre à promouvoir vos projets en ligne.',
    ...
    organizerName: 'EVENTIX Academy',
    organizerEmail: 'education@eventix.com',
    organizerPaypalEmail: 'education@eventix.com',
    maxTicketsPerPurchase: 3,
    status: 'approved'
  },
  {
    name: 'Projection Cinéma D9',
    description: 'Film d’auteur projeté dans une salle intimiste avec débat après la séance.',
    ...
    category: 'cinema',
    subCategory: 'Film',
    organizerName: 'EVENTIX Cinema',
    organizerEmail: 'cinema@eventix.com',
    organizerPaypalEmail: 'cinema@eventix.com',
    maxTicketsPerPurchase: 4,
    status: 'approved'
  }
];

// Only show approved events to customers
app.get('/api/items', async (req, res) => {
  try {
    let items = await Item.find({ status: 'approved' }).sort({ date: 1 });
    
    if (items.length === 0) {
      await Item.insertMany(demoMusicalEvents);
      items = await Item.find({ status: 'approved' }).sort({ date: 1 });
    }
...
// Create event (organizer) - pending approval
app.post('/api/items', authenticateToken, async (req, res) => {
  try {
    ...
    const { 
      name, description, date, endDate, time, venue, city, 
      price, totalSeats, imageUrl, category, subCategory, 
      maxTicketsPerPurchase 
    } = req.body;
    
    if (!name || !date || !time || !venue || !city || price === undefined || totalSeats === undefined) {
      return res.status(400).json({ 
        success: false, 
    ...
    const newItem = new Item({
      ...
      maxTicketsPerPurchase: maxTicketsPerPurchase ? Number(maxTicketsPerPurchase) : 4,
      status: 'pending',
      organizerId: organizer._id,
      organizerName: organizer.name,
      organizerEmail: organizer.email,
      organizerPaypalEmail: organizer.paypalEmail // ✅ ADDED - Store organizer's PayPal
    });
...
app.get('/api/admin/pending-events', authenticateToken, async (req, res) => {
  try {
    ...
    const pendingEvents = await Item.find({ status: 'pending' })
      .sort({ createdAt: -1 })
      .populate('organizerId', 'name email phone paypalEmail');
...
// Get all events (with filter by status for admin)
app.get('/api/admin/events', authenticateToken, async (req, res) => {
  try {
    ...
    const events = await Item.find(filter)
      .sort({ createdAt: -1 })
      .populate('organizerId', 'name email phone paypalEmail');
...
app.put('/api/admin/events/:id/approve', authenticateToken, async (req, res) => {
  try {
    ...
    const event = await Item.findByIdAndUpdate(
      req.params.id,
      {
        status: 'approved',
        approvedAt: new Date()
    ...
    await sendEventStatusEmail(event.organizerEmail, event.name, 'approved');
...
app.put('/api/admin/events/:id/reject', authenticateToken, async (req, res) => {
  try {
    ...
    await sendEventStatusEmail(event.organizerEmail, event.name, 'rejected', reason);
...
// Get organizer's events (with status)
app.get('/api/organizer/events', authenticateToken, async (req, res) => {
  try {
    ...
    const events = await Item.find({ organizerId: req.user.id })
      .sort({ createdAt: -1 });
...
// Send event status email to organizer
async function sendEventStatusEmail(organizerEmail, eventName, status, reason = '') {
  try {
    const subject = status === 'approved' 
      ? `✅ Event "${eventName}" has been Approved!` 
      : `❌ Event "${eventName}" has been Rejected`;
    ...
    const html = status === 'approved'
      ? `
        ...
        <body>
          ...
          <div class="content">
            ...
            <p>Your event "<strong>${eventName}</strong>" has been approved by the admin.</p>
            <p>The event is now live and customers can book tickets.</p>
            <a href="${process.env.BASE_URL || 'http://localhost:3000'}/organizer/dashboard" class="button">View Dashboard</a>
          </div>
        </body>
        </html>
      `
      : `
        ...
        <body>
          ...
          <div class="content">
            <h2>Event Needs Revision</h2>
            <p>Your event "<strong>${eventName}</strong>" has been rejected.</p>
            ${reason ? `<p><strong>Reason:</strong> ${reason}</p>` : ''}
            <p>Please make the necessary changes and resubmit the event.</p>
          </div>
        </body>
        </html>
      `;
    ...
    await emailTransporter.sendMail(mailOptions);
    console.log(`📧 Event status email sent to ${organizerEmail}`);
    return { success: true };
  } catch (error) {
    console.error('Failed to send event status email:', error);
    return { success: false, error: error.message };
  }
}
...
app.put('/api/items/:id', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    const { name, description, date, endDate, time, venue, city, price, totalSeats, imageUrl, category, subCategory, organizerName, organizerEmail, maxTicketsPerPurchase } = req.body;
    ...
    if (totalSeats !== undefined) {
      ...
      availableSeats = Number(totalSeats) - soldSeats;
      if (availableSeats < 0) {
        return res.status(400).json({ success: false, message: 'Le nombre total de places ne peut pas être inférieur au nombre de tickets déjà vendus.' });
      }
    }

    const updatedEvent = await Item.findByIdAndUpdate(
      id,
      {
        $set: {
          name,
          ...
          subCategory: subCategory !== undefined ? subCategory : event.subCategory,
          organizerName: organizerName !== undefined ? organizerName : event.organizerName,
          organizerEmail: organizerEmail !== undefined ? organizerEmail : event.organizerEmail,
          maxTicketsPerPurchase: maxTicketsPerPurchase !== undefined ? Number(maxTicketsPerPurchase) : event.maxTicketsPerPurchase,
          updatedAt: Date.now()
        }
      },
      { new: true }
    ...
    res.json({ success: true, message: 'Événement mis à jour avec succès.', data: updatedEvent });
  } catch (error) {
...
app.get('/api/organizer-dashboard', authenticateToken, async (req, res) => {
  try {
    const tokenUser = req.user || {};
    const roleFromToken = tokenUser.role || '';
    const userEmailFromToken = tokenUser.email || '';

    const { role: roleQuery, email: emailQuery, organizerName } = req.query;
    const isAdmin = String(roleQuery || roleFromToken || '').toLowerCase() === 'admin';
    const organizerEmail = String(emailQuery || userEmailFromToken || '').trim().toLowerCase();
    const organizerNameQuery = String(organizerName || '').trim();

    const itemFilter = {};
    if (!isAdmin) {
      if (organizerEmail) {
        itemFilter.organizerEmail = organizerEmail;
      } else if (organizerNameQuery) {
        itemFilter.organizerName = new RegExp(organizerNameQuery, 'i');
      } else {
        return res.json({
          success: true,
          data: {
    ...
    const events = await Item.find(itemFilter).lean();
    const eventIds = events.map((event) => event._id);

    const bookingFilter = { bookingStatus: { $ne: 'cancelled' } };
    if (!isAdmin) {
      bookingFilter.eventId = { $in: eventIds.length ? eventIds : [null] };
    }

    const bookings = await Booking.find(bookingFilter).lean();

    const totalEventRevenue = bookings.reduce((sum, booking) => sum + (booking.totalPrice || 0), 0);
    const totalAppRevenue = Math.round(totalEventRevenue * 0.1);
    const soldTickets = bookings.reduce((sum, booking) => sum + (booking.totalQuantity || 0), 0);
    const roomCount = new Set(events.map((event) => event.venue)).size;

    const monthlyMap = {};
    bookings.forEach((booking) => {
      const date = new Date(booking.createdAt || booking.updatedAt || Date.now());
      const monthKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
      monthlyMap[monthKey] = monthlyMap[monthKey] || { month: monthKey, eventRevenue: 0 };
      monthlyMap[monthKey].eventRevenue += booking.totalPrice || 0;
    });

    const monthlySuccess = Object.values(monthlyMap).map((entry) => {
      const [year, month] = entry.month.split('-');
      return {
        month: `${month}/${year}`,
        eventRevenue: entry.eventRevenue
      };
    }).sort((a, b) => a.month.localeCompare(b.month));

    const eventRevenueMap = {};
    bookings.forEach((booking) => {
      const key = String(booking.eventId || booking.eventName || booking._id);
      eventRevenueMap[key] = eventRevenueMap[key] || {
      ...
      eventRevenueMap[key].eventRevenue += booking.totalPrice || 0;
      eventRevenueMap[key].soldTickets += booking.totalQuantity || 0;
    });

    const topEvents = Object.values(eventRevenueMap)
      .filter((item) => item.eventRevenue > 0)
      .sort((a, b) => b.eventRevenue - a.eventRevenue)
      .slice(0, 5)
      .map((item, index) => ({
        id: String(item.eventId || item.name || `top-${index}`),
        name: item.name || 'Evenement',
        room: item.room || '',
    ...
    res.json({
      ...
      data: {
        ...
        events: events.map((event) => ({ ...event, id: String(event._id) })),
        monthlySuccess,
        topEvents
      }
    });
