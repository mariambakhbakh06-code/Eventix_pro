...
const Events = () => {
    ...
    useEffect(() => {
        ...
        // If there is no authenticated user, fall back to contact info if available.
        ...
        if (storedContact) {
            try {
                ...
                setContactPerson({ firstName: "", lastName: "", email: "", phone: "" });
                setIsSignedIn(false);
            }
        } else {
            setContactPerson({ firstName: "", lastName: "", email: "", phone: "" });
    ...
    const openBookingModal = (event, quantity) => {
        ...
        // Reset payment step when opening modal
    ...
    // MODIFIED: Validate and move to payment step instead of submitting directly
    const submitBooking = async () => {
        ...
        // Store booking data and show payment step
        setPendingBookingData({
            customerEmail: contactPerson.email,
            eventId: selectedEvent._id,
            attendees: attendees,
            totalAmount: selectedEvent.price * bookingQuantity
        });
    ...
    // NEW: Handle successful payment and create booking
    const handlePaymentSuccess = async (paymentResult) => {
        setPaymentStatus({ loading: true });
        
        try {
            // Now that payment is complete, create the booking
            ...
            setPaymentStatus({
                ...
                message: `✅ Payment successful! Your ticket code: ${result.data.ticketCode}\nCheck your email for confirmation.`
            });
            ...
            setPaymentStatus({ error: err.message || "Booking failed after payment. Please contact support." });
        } finally {
    ...
    return (
        <div style={{ padding: "24px", fontFamily: "sans-serif" }} className="page-enter">
            <h1 className="EVENTIX">EVENTIX</h1>
            
            <div className="carousel">
                <div className="carousel-track">
                    {loopEvents.map((event, index) => (
                        <div className="carousel-item" key={index}>
                            <img src={event.imageUrl || "https://images.unsplash.com/photo-1501281668745-f7f57925c3b4?auto=format&fit=crop&w=800&q=80"} alt={event.name} />
            ...
            <div className="search-container">
                ...
                <input
                    ...
                    placeholder="Rechercher un événement, lieu, ville, sous-catégorie..."
                    value={searchQuery}
            ...
            {!loading && !error && filteredEvents.length === 0 && <p style={{ textAlign: "center", color: "#aaa" }}>Aucun événement trouvé.</p>}
            ...
            {!isSignedIn && !loading && (
                <div style={{
                    ...
                    <p style={{ margin: 0 }}>
                        ⚠️ Please <a href="/signin" style={{ fontWeight: "bold", color: "#007bff" }}>sign in</a> to book tickets and manage your bookings.
                    </p>
                </div>
            )}
                    ...
                    <div key={event._id} className="event_card">
                            ...
                            {event.category && (
                                <div style={{ marginBottom: "12px" }}>
                                    <span style={{
                                        ...
                                        fontWeight: "bold",
                                        backgroundColor: 
                                            event.category === 'sport' ? '#e3f2fd' :
                                            event.category === 'musical' ? '#f3e5f5' :
                                            event.category === 'cinema' ? '#fff3e0' : '#e8f5e9',
                                        color:
                                            event.category === 'sport' ? '#1976d2' :
                                            event.category === 'musical' ? '#7b1fa2' :
                                            event.category === 'cinema' ? '#e65100' : '#2e7d32'
                                    }}>
                                        🎯 {event.category.toUpperCase()}
                                        {event.subCategory ? ` / ${event.subCategory.toUpperCase()}` : ''}
                                    </span>
                                </div>
                            )}
                        ...
                        <button
                            ...
                            style={{
                                ...
                                marginTop: "12px",
                                padding: "10px",
                                backgroundColor: event.availableSeats === 0 ? "#ccc" : "#007bff",
                                color: "white",
                                border: "none",
                                borderRadius: "6px",
            ...
            {/* Booking Modal with PayPal Integration */}
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <h2 style={{ margin: 0 }}>Complete Your Booking</h2>
                            <button 
                                onClick={closeModal}
                                disabled={bookingStatus.loading || paymentStatus?.loading}
                                style={{
                                    background: "#dc3545",
                                    border: "none",
                                    color: "white",
                        ...
                        {!paymentStep ? (
                            ...
                            <>
                                ...
                                {attendees.map((attendee, index) => (
                                    <div key={index} style={{
                                        ...
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
                                            <h4 style={{ margin: 0, color: "#007bff" }}>Attendee {index + 1}</h4>
                                            <button
                                                type="button"
                                ...
                                <div style={{ 
                                    ...
                                    <button
                                        ...
                                        style={{
                                            flex: 1,
                                            padding: "12px",
                                            backgroundColor: bookingStatus.loading ? "#ccc" : "#007bff",
                                            color: "white",
                                            border: "none",
                                            borderRadius: "6px",
                            ...
                            <>
                                <div style={{
                                    ...
                                    <h3 style={{ margin: 0, color: "#1976d2" }}>💳 Complete Payment</h3>
                                    <p style={{ margin: "10px 0 0 0", fontSize: "14px" }}>
                                        Total to pay: <strong>{selectedEvent.price * bookingQuantity} DH</strong>
                                    </p>
                                </div>
                                ...
                                {!paymentStatus?.success && (
                                    <PayPalScriptProvider options={{ 
                                        "client-id": "AQf0VPszk-PUWp8Kvjaz-50K7xUX2jVJzgO1fWXThUBCcuVVnR1y5rmEA-uc-rx5Azr_eFtjA4ma08KC", // Replace with your actual sandbox client ID
                                        "currency": "USD",
                                        "intent": "capture"
                                    }}>
                                        <PayPalButtons
                                            createOrder={async () => {
                                                try {
                                                    const response = await fetch("http://localhost:5000/api/paypal/create-order", {
                                                        ...
                                                        headers: { 'Content-Type': 'application/json' },
                                                        body: JSON.stringify({
                                                            amount: (selectedEvent.price * bookingQuantity).toString(),
                                                            currency: 'USD',
                                                            description: `${bookingQuantity} ticket(s) for ${selectedEvent.name}`
                                                        })
                                                    });
                                                    ...
                                                    setPaymentStatus({ error: "Failed to initialize payment" });
                                                }
                                            }}
                                            onApprove={async (data, actions) => {
                                                try {
                                                    ...
                                                    if (result.success) {
                                                        ...
                                                        setPaymentStatus({ error: "Payment capture failed" });
                                                    }
                                                } catch (error) {
                                            ...
                                            onError={(err) => {
                                                ...
                                                setPaymentStatus({ error: "PayPal error. Please try again." });
                                            }}
                                        />
