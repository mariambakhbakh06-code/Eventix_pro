...
const Cinema = () => {
    ...
    // Filter cinema events
    const cinemaEvents = events.filter(
        (event) => event.category?.toLowerCase() === "cinema"
    );
    ...
    useEffect(() => {
        if (cinemaEvents.length === 0) return;
    ...
    useEffect(() => {
        if (currentIndex === cinemaEvents.length) {
            setTimeout(() => {
                setCurrentIndex(0);
            }, 800);
        }
    }, [currentIndex, cinemaEvents.length]);

    const loopEvents = [...cinemaEvents, ...cinemaEvents];

    return (
        <div style={{ padding: "24px", fontFamily: "sans-serif" }} className="page-enter">
            <h1 className="EVENTIX">EVENTIX CINEMA</h1>
            ...
            {loading && <p>Loading cinema events...</p>}
            {error && <p style={{ color: "red" }}>{error}</p>}
            {!loading && !error && cinemaEvents.length === 0 && (
                <p style={{ textAlign: "center", fontSize: "18px", marginTop: "40px" }}>
                    No cinema events available at the moment.
                </p>
            )}

            {!isSignedIn && !loading && cinemaEvents.length > 0 && (
                <div style={{
                    ...
                    <p style={{ margin: 0 }}>
                        ⚠️ Please <a href="/signin" style={{ fontWeight: "bold", color: "#007bff" }}>sign in</a> to book cinema tickets.
                    </p>
                </div>
            )}
                    ...
                    <div key={event._id} className="event_card">
                            ...
                            {event.category && (
                                <div style={{ marginBottom: "12px" }}>
                                    <span style={{
                                        ...
                                        🎬 CINEMA {event.subCategory ? `/ ${event.subCategory.toUpperCase()}` : ''}
                                    </span>
                                </div>
                            )}
                        ...
                        <button
                            ...
                            style={{
                                ...
                                marginTop: "12px",
                                padding: "10px",
                                backgroundColor: event.availableSeats === 0 ? "#ccc" : "#e65100",
                                color: "white",
                                border: "none",
                                borderRadius: "6px",
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <h2 style={{ margin: 0 }}>Complete Your Cinema Booking</h2>
                            <button 
                                onClick={closeModal}
                        ...
                        <div style={{
                            ...
                            <p><strong>Movie:</strong> {selectedEvent.name}</p>
                            <p><strong>Date:</strong> {new Date(selectedEvent.date).toLocaleDateString()}</p>
                            <p><strong>Time:</strong> {selectedEvent.time}</p>
                            <p><strong>Cinema:</strong> {selectedEvent.venue}, {selectedEvent.city}</p>
                        ...
                        {attendees.map((attendee, index) => (
                            <div key={index} style={{
                                ...
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
                                    <h4 style={{ margin: 0, color: "#e65100" }}>Attendee {index + 1}</h4>
                                    <button
                                        type="button"
                        ...
                        <div style={{ display: "flex", gap: "10px", marginTop: "20px" }}>
                            ...
                            <button
                                ...
                                style={{
                                    flex: 1,
                                    padding: "12px",
                                    backgroundColor: bookingStatus.loading ? "#ccc" : "#e65100",
                                    color: "white",
                                    border: "none",
                                    borderRadius: "6px",
