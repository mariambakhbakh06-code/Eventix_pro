...
const OrganizerSignIn = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        earnings: "90",
        acceptTerms: false,
        paypalEmail: ""  // ✅ ADDED - PayPal email field
    });
    ...
    const validateForm = () => {
        ...
        if (!formData.paypalEmail.trim()) {
            newErrors.paypalEmail = "PayPal email is required to receive payments";
        } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(formData.paypalEmail)) {
        ...
        try {
            const response = await fetch(`${API_URL}/api/organizers/signin`, {
                ...
                body: JSON.stringify({
                    ...
                    paypalEmail: formData.paypalEmail.trim()  // ✅ ADDED - Send PayPal email to backend
                }),
            });
            ...
            setFormData({
                ...
                earnings: "90",
                acceptTerms: false,
                paypalEmail: ""  // ✅ Reset PayPal email
            });
                ...
                <form onSubmit={handleSubmit} className="signin-form">
                    ...
                    <div className="form-group">
                        ...
                        <input
                            ...
                            onChange={handleChange}
                            disabled={isLoading}
                            className={errors.paypalEmail ? "error-input" : ""}
                        />
                        {errors.paypalEmail && (
                            <span className="error">
                                {errors.paypalEmail}
                        ...
                        <small className="help-text" style={{ display: "block", marginTop: "5px", color: "#666" }}>
                            This is where EVENTIX will send your ticket revenue after each sale
                        </small>
                    </div>
